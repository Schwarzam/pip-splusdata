...
def get_fits(name, ra, dec, cut_size, band, filename=None):
    ...
    response = urlopen(f'http://143.107.18.89:8000/api/{name}/{ra}/{dec}/{cut_size}/{band}/0')
    name_in_query = response.read().decode('utf-8')

    response = urlopen(f'http://143.107.18.89:8000/api/{name_in_query}/{ra}/{dec}/{cut_size}/{band}/1')
    ...
    if int(queue) == 0:
        response = urlopen(f'http://143.107.18.89:8000/api/{name_in_query}/{ra}/{dec}/{cut_size}/{band}/2')
        queue = response.read().decode('utf-8')
